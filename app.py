...
from agent_core import add_new_task, get_all_tasks, delete_task, update_task, get_audit_logs, init_db
...
def load_data():
    ...
    if not df.empty: df['deadline'] = pd.to_datetime(df['deadline']).dt.date
    return df
...
with tabs[1]:
    if st.button("توليد تقرير أداء ذكي"):
        ...
        # استخدام الموديل المعتمد الجديد
        model = genai.GenerativeModel('gemini-1.5-flash')
        st.write(model.generate_content(f"حلل أداء الفريق بناءً على البيانات:\n{df_raw.to_string()}").text)
...
with tabs[2]:
    # منطق الإدارة (نفس الكود السابق، تأكدي من وضعه هنا)
    st.write("إدارة المهام")
